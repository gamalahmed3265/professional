document.addEventListener("DOMContentLoaded", function () {
  ...
  function toggleMobileMenu() {
    if (mobileMenuDrawer.classList.contains("left-0")) {
  ...
  const academicProgramsMobileToggle = document.getElementById(
    "academic-programs-mobile-toggle"
  );
  const academicProgramsMobileDropdown = document.getElementById(
    "academic-programs-mobile-dropdown"
  );
  academicProgramsMobileToggle.addEventListener("click", () => {
    academicProgramsMobileDropdown.classList.toggle("hidden");
    academicProgramsMobileToggle
      .querySelector(".fa-chevron-down")
      .classList.toggle("rotate-180");
  });

  const coursesMobileToggle = document.getElementById("courses-mobile-toggle");
  const coursesMobileDropdown = document.getElementById(
  ...
  faqToggles.forEach((toggle) => {
    toggle.addEventListener("click", () => {
      ...
      faqToggles.forEach((otherToggle) => {
        if (
          otherToggle !== toggle &&
          !otherToggle.nextElementSibling.classList.contains("hidden")
        ) {
